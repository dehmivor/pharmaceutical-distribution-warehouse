...
export default function RepresentativeTopRef({ data }) {
  ...
  const chartData = data?.map(item => {
    ...
    return {
      date: date,
      'Export Orders': Number(item.count || 0),
      'Total Value': Number(item.value || 0)
    };
  }) || [];
        ...
        <Box sx={{ mb: 2, display: 'flex', alignItems: 'center', gap: 1 }}>
          ...
          <Box>
            ...
            <Typography variant="body2" color="text.secondary">
              Monthly statistics for all representatives
            </Typography>
          </Box>
        </Box>
  ...
  return (
    <MainCard>
      ...
      <Box sx={{ height: '300px', width: '100%' }}>
                           <LineChart
                     dataset={chartData.length > 0 ? chartData : [{ date: new Date(), 'Export Orders': 0 }]}
                     xAxis={[{ 
                       dataKey: 'date', 
                       scaleType: 'point',
