...
export default function ManageExportOrders() {
  ...
  const addInternalLine = () => {
    setInternalLines((prev) => [
      ...prev,
      { medicine_id: "", destroy_total: 0, picked: [] },
    ])
  }
  ...
  const addPickPackage = (lineIndex, packageId) => {
    setInternalLines((prev) => prev.map((l, i) => {
  ...
  const removePickPackage = (lineIndex, packageId) => {
    setInternalLines((prev) => prev.map((l, i) => (i === lineIndex ? { ...l, picked: l.picked.filter((p) => p.package_id !== packageId) } : l)))
  }
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      <Dialog open={internalDialogOpen} onClose={closeInternalDialog} maxWidth="md" fullWidth>
                    ...
                    <Stack spacing={1.5}>
                      {pkgList.map((pkg) => {
                        ...
                        return (
                          ...
                          >
                            <Typography variant="body2" flex={1}>
                              {pkg.batch.batch_code} | Tồn: {pkg.quantity} | Vị trí: {pkg.location.area_name || ""}-{pkg.location.bay || ""}-{pkg.location.row || ""}-{pkg.location.column || ""}
                            </Typography>
                            {picked ? (
                              <>
                                <TextField
        ...
        <DialogActions>
          <Button variant="outlined" onClick={closeInternalDialog}>Đóng</Button>
