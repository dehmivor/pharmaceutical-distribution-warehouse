...
const { annexSchema } = require('./subSchemas');
const { CONTRACT_STATUSES, PARTNER_TYPES } = require('../utils/constants');

const principalContractSchema = new mongoose.Schema({
  contract_code: {
    type: String,
    unique: true,
    trim: true,
  ...
  annexes: [annexSchema], // Phụ lục hợp đồng
  status: {
    type: String,
...
module.exports = mongoose.model('PrincipalContract', principalContractSchema);